...
void post_api(double temperatura, int luminosidade, double umidade) {
  if (WiFi.status() == WL_CONNECTED) { 
    HTTPClient http;
    ...
    String postData = "{\"temperatura\":" + String(temperatura, 2) + ", \"luminosidade\":" + String(luminosidade) + + ", \"umidade\":" + String(umidade) + "}";
...
void loop() {
  ...
  // Post
  post_api(getTemperatureCelsius(tempValue), getPercentLight(luzValue), getPercentHumidity());
